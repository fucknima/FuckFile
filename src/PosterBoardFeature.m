...
static NSString *PBWorkspace(void)
{
    return [MCMVirtualRoot()
        stringByAppendingPathComponent:MCMWallpaperLabName()];
}
...
static NSDictionary *PBPosterBoardContext(NSError **error)
{
    ...
    NSString *root = [[MCMManager sharedManager]
        dataContainerPathForIdentifier:PBPosterBoardIdentifier error:&activationError];
    if (!root) {
