...
1. 任务历史此前点击无响应。现在仅 completed 的任务可点：下载 → 目标目录并
   定位最终文件（detailName）、单源复制/移动 → 定位结果文件、多源 → 打开
   目标目录、解压 → 打开解压出的目录、压缩 → 定位生成的压缩包；失败/取消/
   进行中不跳转，文件已不存在时提示「目标文件已不存在」。
2. 跳转由 FFRootTabBarController 统一执行：关闭任务中心 → 切到「文件」tab →
   复用浏览栈中同目录的浏览器（没有就 push 一个），并设置
   FFBrowserViewController.pendingRevealPath；目录加载完成后滚动定位，列表
   模式短暂选中高亮，网格模式只滚动（网格没有选中态样式）。
3. 已完成的历史行显示 disclosure 指示器；进行中的行仍只有进度与取消按钮。

原因：用户反馈任务中心里下载完成的文件点击无反应。
...
**任务中心下载条目显示实际文件名；存档绝对路径按当前容器重写。**

1. 下载任务的标题此前一直是 URL 尾段（常见为哈希/token）。现在下载成功后把
   displayName 更新为「下载 <最终文件名>」，任务中心/胶囊显示的就是真实文件
   名；detailName 继续作为跳转定位依据。
2. 修复「文件明明在，点击却说不存在」：更新/重装后 App 容器 UUID 变化，而
   任务历史、书签、最近列表里存的是旧容器的绝对路径。FFCanonicalStoragePath
   现在会把失效的「…/Containers/Data/Application/<旧 UUID>/…」按「当前容器 +
   相同后缀」映射回来，只在原路径不存在且映射后的路径真实存在时改写；任务
   跳转、书签、最近列表同时受益。
3. 任务跳转解析失败时写日志（kind/destination/name），便于真机定位。

原因：用户反馈任务中心里下载完成的文件点击后提示不存在、文件名是一串哈希。
...
1. 网页下载条的「查看任务」此前 push 一个没有接线的任务中心实例，点已完成
   任务没有任何反应；现在统一调用 FFRootTabBarController.presentTaskCenter
   （模态 + 跳转接线），所有入口行为一致。
2. 点击非 completed 任务不再静默：进行中提示「任务还在进行中」，失败/取消
   提示「任务未完成，没有可跳转的文件」。
3. 已完成任务的目标文件缺失但所在目录仍在时，不再静默打开目录：跳转后用
   toast 说明「未找到 <name>，已打开所在目录」；下载还会用 URL 尾段兜底再找
   一遍文件名。每次点击写日志（state/kind/destination/detail）便于真机定位。

原因：用户反馈第二次打开后点击任务无反应，日志里没有任何跳转记录。
...
**合并下载入口：只保留「＋ → 下载…」的内置浏览器。**

1. 「从 URL 下载…」弹窗与「从网页下载…」浏览器是包含关系：前者只认 https、
   不带 Cookie/Referer、处理不了登录与跳转。现在删除弹窗与
   `startDownloadFromURL:`，＋菜单只留「下载…」（页面标题同名校为「下载」），
   进入后自动聚焦地址栏。
2. 直链：粘到地址栏回车，响应阶段（不可显示 MIME / attachment）自动转任务。
   可显示内容（图片/PDF/文本）不会自动转下载，因此导航栏新增「下载当前页面」
   按钮，显式把当前 URL 交给任务系统（同样带 Cookie/Referer/UA），
   保证「从 URL 下载」原有的能力不丢。
...
**下载浏览器支持「深色浏览」（所有网页可强制暗色）。**

1. 导航栏新增月亮开关：默认跟随系统（系统深色即开启），手动切换后写入
   NSUserDefaults（FFWebBrowserForceDark）持久化；切换即时作用于当前页面
   （页内 __ffSetForceDark）并重新注入 user script 供后续导航使用。
2. 渲染方式：文档结束注入脚本——站点自带深色（body/html 有效背景亮度 < 0.4）
   不动；透明画布按白底处理；其余页面用 html{filter:invert(1) hue-rotate(180deg)}
   强制暗色，图片/视频/iframe 反向一次保持原色。已知代价：filter 会创建包含块，
   个别站点 position:fixed 元素可能不再吸顶，可随时用开关关掉。
3. 强制深色时 WKWebView 的 underPageBackgroundColor 同步压暗（#121212），
   避免透明背景页面反转后白字压白底；关闭时回到系统背景。
