...
> - 导航栏固定 `＋（唯一创建：新建文件夹/文件）+ …`；普通模式移除底部
>   工具栏（筛选/新建不再常驻），底部工具栏只属于多选模式
> - 对象级菜单单一来源：`contextMenuSectionsForEntry:` 分区（打开/查看 ‖
>   复制·剪切·副本·重命名·收藏·分享 ‖ 压缩·浏览压缩包·解压·安装 ‖
>   用其他查看器打开 ‖ 复制路径·属性·删除）；长按菜单 / Grid 长按 /
>   左滑「更多」共用同一份定义；左滑只留 删除 + 更多
> - 多选底栏：复制/移动/分享/更多（含压缩）/删除；全选只在导航栏
> - Grid 补齐：长按菜单、缩略图、下拉刷新、空态/加载/错误；统一
>   `refreshVisibleContent` 刷新（List/Grid/空态一起刷新）
> - Archive 浏览器与 Browser 同语言：`…`（选择/全部解压/分享压缩包）+
>   选择模式 取消/全选/底栏「提取」
> - 新增 FFViewerPickerViewController：列表式查看器选择（默认关联打勾，
>   选中即设覆盖并立即打开），用于 Browser「用其他查看器打开」与
>   设置页「文件关联」编辑，替代两处超长 Action Sheet
