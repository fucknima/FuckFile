...
- SQLite3 编辑器（sqlite/sqlite3/sqlitedb/db：库信息、表、视图、索引、
  schema、分页浏览、SQL 查询）
- IPA 包信息（ZIP 包内浏览、图标与元数据读取；不提供安装）
...
- `＋ → 下载…`：内置下载浏览器。直链粘到地址栏即可（文件响应自动转任务，
  可显示的内容用导航栏「下载当前页面」显式落盘）；需要登录的站点在同一界面
  登录后下载。Cookie/Referer/UA 随任务携带。
...
- 下载完成通知
- 下载目录管理
- 深色浏览：默认跟随系统，可手动强制所有网页暗色（导航栏月亮开关）
