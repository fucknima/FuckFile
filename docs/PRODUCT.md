...
- 文件夹导航
- 返回上级
- 路径导航（单行语义面包屑：设备存储 › AppData › 微信 › Documents，
  上级目录可点击直达，不展示 /private/var/... 原始前缀）
- 文件列表
- 网格模式（自适应列数：按宽度/字号自动计算，不写死列数；
  浏览器「更多 → 显示方式」随时切换，设置「默认视图」控制新目录默认值）
- 列表模式
- 文件夹优先显示选项
- 显示隐藏文件
- 文件数量显示
...
链接目标/权限/xattr 等高级信息只出现在属性页（目录扫描阶段不读取）。
