...
// Share extensions cannot open their containing app with public API: Apple's
// NSExtensionContext.open only supports the Today and iMessage extension
// points, so the system ignores the documented call here. The handoff is the
// point of this extension, therefore the LaunchServices/UIApplication
// best-effort paths are kept; the public call stays as the last resort.
...
- (BOOL)openWakeURL:(NSURL *)url
{
    if (!url) return NO;
    if ([self openWakeURLViaLaunchServices:url]) return YES;
    if ([self openWakeURLViaUIApplication:url]) return YES;
    [self.extensionContext openURL:url completionHandler:^(BOOL success) {
        NSLog(@"[FuckFileShare] wake via extensionContext=%d", success);
    }];
    return NO;
}
...
- (void)finishWithImportedCount:(NSInteger)count
{
    ...
    if (self.bridgeUsesAppGroup) {
        ...
        BOOL requested = [self openWakeURL:wakeURL];
