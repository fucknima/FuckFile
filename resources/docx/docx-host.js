(() => {
  ...
  const setZoom=(v,emit=false)=>{zoom=Math.max(50,Math.min(250,Math.round(v/10)*10));doc.style.zoom=String(zoom/100);doc.style.width=`${10000/zoom}%`;document.getElementById('zoom').textContent=`${zoom}%`;if(emit)emitState(true);};
  ...
  window.FFDocx={captureState,restoreState};
